...
An installer is also available, designed to deploy AIIDA together with its dependent services.
However, the active development of AIIDA and its related services has been postponed for future use cases.

While the Helm chart remains functional, note that its secrets are originally generated by the installer.
