...
- **source**: List of IDs of other data-points referenced in the transform expression.
...
- **write**: Value can be written to the device (currently not supported).
- **readwrite**: Bi-directional (currently not supported).


## Notes

- Use `::` to reference data-points across sources, e.g., `@battery-1::state_of_charge_lit`.
- Virtual data-points support chained expressions combining multiple data sources.
