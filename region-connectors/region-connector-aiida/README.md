...
| `region-connector.aiida.kafka.bootstrap-servers`        | List of Kafka bootstrap servers, to which AIIDA instances should send the near real-time data and status messages. The region-connector also connects to this broker to listen for connection status messages. By default, the same value as specified for `kafka.bootstrap.servers` is used.                         |
