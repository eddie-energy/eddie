...
public class NotOlderThanValidator implements Validator<EsPermissionRequest> {
     ...
     * Uses {@link DatadisRegionConnectorMetadata#ZONE_ID_SPAIN} as timezone for the comparison timestamp.
    ...
    public List<AttributeError> validate(EsPermissionRequest value) {
        var earliestAllowedStart = ZonedDateTime.now(DatadisRegionConnectorMetadata.ZONE_ID_SPAIN).minus(limit, unit);
