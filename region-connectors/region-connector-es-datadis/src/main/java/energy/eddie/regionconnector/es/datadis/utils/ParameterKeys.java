package energy.eddie.regionconnector.es.datadis.utils;

public class ParameterKeys {

    public static final String PERMISSION_ID_KEY = "permissionId";
    public static final String CONNECTION_ID_KEY = "connectionId";

    public static final String NIF_KEY = "nif";
    public static final String METERING_POINT_ID_KEY = "meteringPointId";
    public static final String REQUEST_DATE_FROM_KEY = "start";
    public static final String REQUEST_DATA_TO_KEY = "end";
    public static final String MEASUREMENT_TYPE_KEY = "measurementType";

    private ParameterKeys() {
    }
}
