...
public record EdaCMNotification01p20(CMNotification notification) implements EdaCMNotification {
    @Override
    ...
    public List<ResponseData> responseData() {
        return notification.getProcessDirectory().getResponseData().stream()
                           .map(responseData -> (ResponseData) new ResponseData01p20(responseData))
                           .toList();
    }
}
