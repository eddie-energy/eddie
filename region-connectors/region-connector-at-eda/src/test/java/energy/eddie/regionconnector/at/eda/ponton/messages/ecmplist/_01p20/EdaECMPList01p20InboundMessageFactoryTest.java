...
class EdaECMPList01p20InboundMessageFactoryTest {
    @Autowired
    ...
    void parseInputStream_parsesMessagesAsExpected() throws IOException {
        // Given
        ClassLoader classLoader = EdaECMPList01p20InboundMessageFactory.class.getClassLoader();
        var inputStream = classLoader.getResourceAsStream("xsd/ecmplist/_01p20/ecmplist.xml");
        ...
        // Then
        assertAll(
                () -> assertEquals("123456789", ecmpList.messageId()),
                () -> assertEquals("0ASDF", ecmpList.conversationId()),
                () -> assertEquals("AT00100000000RC100123000000123456", ecmpList.ecId())
        );
    }
    ...
    private static Stream<Arguments> activeDates() {
        return Stream.of(
                Arguments.of(LocalDate.of(2026, 10, 5)),
