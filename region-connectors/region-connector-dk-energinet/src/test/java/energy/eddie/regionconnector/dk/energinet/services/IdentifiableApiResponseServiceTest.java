...
class IdentifiableApiResponseServiceTest {
    ...
    private final LocalDate periodEnd = ZonedDateTime.parse(marketDocumentResponse.getMyEnergyDataMarketDocument()
                                                                                  .getPeriodTimeInterval()
                                                                                  .getEnd())
                                                     .withZoneSameInstant(DK_ZONE_ID)
                                                     .toLocalDate();
    private final LocalDate periodStart = ZonedDateTime.parse(marketDocumentResponse.getMyEnergyDataMarketDocument()
                                                                                    .getPeriodTimeInterval()
                                                                                    .getStart())
                                                       .withZoneSameInstant(DK_ZONE_ID)
                                                       .toLocalDate();
    ...
    public static MyEnergyDataMarketDocumentResponse readMyEnergyDataMarketDocumentResponseListApiResponse() throws IOException {
        try (InputStream is = IdentifiableApiResponseServiceTest.class.getClassLoader()
                                                                      .getResourceAsStream(
                                                                              "MyEnergyDataMarketDocumentResponseListApiResponse.json")) {
            return objectMapper.readValue(Objects.requireNonNull(is),
    ...
    void ifLastPolledEqualsStart_callsUpdateLastPolled() {
        // Given
        DkEnerginetCustomerPermissionRequest permissionRequest = new SimplePermissionRequest(periodStart,
                                                                                             periodEnd,
                                                                                             periodStart);
    ...
    void periodEndAfterPermissionEndDate_callsFulfillOnlyOnce() throws StateTransitionException {
        ...
        StepVerifier.create(testPublisher)
                    .then(() -> {
                        testPublisher.emit(new IdentifiableApiResponse(spy, marketDocumentResponse),
                                           new IdentifiableApiResponse(spy, marketDocumentResponse));
