...
| `onopen`                            | Function | Callback function that is called when the button is clicked. Example: `onopen="console.log(Dialog opened)"`.         | No       |
| `onclose`                           | Function | Callback function that is called when the dialog is closed. Example: `onclose="console.log(Dialog close)"`.          | No       |
...
The status of the user interaction can be tracked using events dispatched by the `eddie-connect-button` element.
...
| MANAGEMENT_SERVER_PORT                                               | Port for the management api (only available if _EDDIE_DATA_NEEDS_CONFIG_DATA_NEED_SOURCE=DATABASE_).                                                                                                                                                                                                           |
