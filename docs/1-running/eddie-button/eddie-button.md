...
| `accounting-point-id`               | String  | Sets a fixed accounting point id (metering point ID, POD, CUPS) for permission administrators that require it.                                                                   |
| `customer-identification`           | String  | Sets a fixed customer identification for permission administrators that require it.                                                                                              |
| `meter-id`                          | String  | ID of a physical smart meter asset (serial number) passed to AIIDA permission requests to map flexible connection agreements and control data source selection.                  |
| `min-limit-kw`                      | Number  | Passed to AIIDA permission requests to set a default minimum energy consumption or production limit in kilowatts for controllable units.                                         |
| `max-limit-kw`                      | Number  | Passed to AIIDA permission requests to set a default maximum energy consumption or production limit in kilowatts for controllable units.                                         |
...
Permission request states are described in [integrating](../../2-integrating/permission-requests.md#permission-process-model).
