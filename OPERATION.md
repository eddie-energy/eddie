...
|----------------|--------|------------------------------------------------------|
| `eddieUrl`     | URL    | Public base URL of the EDDIE framework               |
...
The `connectionId` has to be generated by the EP application. It's included in the messages sent by the EDDIE Framework
...
id must be used only once to create a uniquely identifiable connection with the EDDIE framework.
...
- `application.properties`: A base configuration is provided inside of the application, but additional values are
  required that can be supplied in the EDDIE Framework's runtime directory (`./application.properties`). The base
  configuration is located
  at [framework/src/main/resources/application.properties](https://github.com/eddie-energy/eddie/blob/main/framework/src/main/resources/application.properties).

### Business domain releated configuration
...
- A request to `HISTORICAL_VALIDATED_CONSUMPTION_DATA` and `FUTURE_VALIDATED_CONSUMPTION_DATA` leads to the same data
  structure delivered by the EDDIE Framework. But the process of requesting these kinds of data differ, so it's
